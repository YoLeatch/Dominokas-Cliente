...
const DEFAULT_API_URL: &str = match std::option_env!("DOMINOKAS_API_URL") {
    Some(url) => url,
    None => "https://api.dominokas.com",
};
...
fn target_dir_for(kind: &str, game_dir: &Path) -> Result<PathBuf, String> {
    ...
    match kind {
        ...
        "addon" => Ok(citadel.join("dominokas_addons").join("vpks")),
...
pub async fn prepare_and_connect(
    ...
    let game_dir = find_game_dir()?;
    let addons_dir = game_dir.join("citadel").join("dominokas_addons").join("vpks");
