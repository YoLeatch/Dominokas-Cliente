...
pub fn is_valid_ip_port(value: &str) -> bool {
    ...
    let port_valid = match port.parse::<u16>() {
        Ok(p) => p > 0,
        Err(_) => false,
    };
    if !port_valid { return false; }

    // Se for IPv4, valida os octetos
    let octets: Vec<&str> = host.split('.').collect();
    let is_ipv4 = octets.len() == 4 && octets.iter().all(|o| o.parse::<u8>().is_ok());
    
    // Se não for IPv4, verifica se parece um hostname válido (letras, números, pontos, hifens)
    let is_hostname = host.chars().all(|c| c.is_alphanumeric() || c == '.' || c == '-');
...
mod tests {
    ...
    fn parses_connect_with_id() {
        match parse_url("dominokas://connect/abc123DEF456") {
    ...
    fn parses_connectip_with_port() {
        match parse_url("dominokas://connectip/1.2.3.4:27015") {
    ...
    fn rejects_connectip_without_port() {
        match parse_url("dominokas://connectip/1.2.3.4") {
    ...
    fn rejects_bad_octet() {
        match parse_url("dominokas://connectip/999.0.0.1:27015") {
            DeepLinkPayload::Error(_) => {}
            other => panic!("expected Error, got {:?}", other),
        }
    }
    ...
    fn rejects_unknown_action() {
        match parse_url("dominokas://foo/bar") {
            DeepLinkPayload::Error(_) => {}
            other => panic!("expected Error, got {:?}", other),
        }
    }
